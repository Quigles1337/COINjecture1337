#!/usr/bin/env python3
"""
Fix P2P consensus service with proper initialization.
"""

import os

def fix_p2p_consensus():
    """Fix the P2P consensus service with correct initialization."""
    try:
        print("🔧 Fixing P2P consensus service initialization...")
        
        # Create a properly formatted P2P consensus service
        p2p_consensus_content = '''#!/usr/bin/env python3
"""
...
'''
        ...
        # Write the fixed P2P consensus service
        with open('/opt/coinjecture-consensus/p2p_consensus_service_fixed.py', 'w') as f:
            f.write(p2p_consensus_content)
        
        # Make it executable
        os.chmod('/opt/coinjecture-consensus/p2p_consensus_service_fixed.py', 0o755)
        
        # Update systemd service to use the fixed version
...
ExecStart=/opt/coinjecture-consensus/.venv/bin/python3 p2p_consensus_service_fixed.py
Restart=always
RestartSec=5
...
'''
        ...
        os.system('systemctl stop coinjecture-p2p-consensus')
        os.system('systemctl start coinjecture-p2p-consensus')
        
        print("✅ P2P consensus service fixed and restarted")
        return True
        
    except Exception as e:
        print(f"❌ Error fixing P2P consensus service: {e}")
        return False

if __name__ == "__main__":
    print("🔧 Fixing P2P consensus service...")
    success = fix_p2p_consensus()
    if success:
        print("✅ P2P consensus service fixed successfully")
    else:
        print("❌ Failed to fix P2P consensus service")
        exit(1)
